...
namespace DAL.UOW.Repositories
{
    class ResponseRepository : IResponseRepository
    {
        ...
        {
            string query = @"SELECT TOP 10 * FROM Response AS r INNER JOIN Member AS m ON r.MemberId = m.Id ORDER BY r.SentOn DESC";
        ...
        {
            string query = @"SELECT TOP 10 * FROM Response AS r INNER JOIN Member AS m ON r.MemberId = m.Id WHERE r.TopicId = @TopicId ORDER BY r.SentOn DESC";
